...
def run_job(args, i, logging):
    ...
    if args.train:
        Global_acc,Glob_loss=server.train(args)
        server.test()
        # 創建橫軸的Round number列表，與Global_acc列表的長度相同
        Round_number = range(1, len(Global_acc) + 1)

        # 繪製折線圖
        plt.plot(Round_number, Global_acc, marker='o')
        ...
        # 將圖片保存為global_acc.png
        plt.savefig("global_acc.png")

        plt.clf()
        
        # 繪製折線圖
        plt.plot(Round_number, Glob_loss, marker='o')
