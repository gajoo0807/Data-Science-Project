...
class Server:
    ...
    def aggregate_parameters(self):
        ...
        # Set all parameters of the global model to zero
        for param in self.model.parameters():
            param.data = torch.zeros_like(param.data)
        total_train = 0


        # Calculate the total number of training samples across all selected users
        for user in self.selected_users:
            total_train += user.train_samples

        # Aggregate the parameters from each selected user based on the number of their training samples
        for user in self.selected_users:
    ...
    def select_users(self, round, num_users):
        ...
        if(num_users == len(self.users)):
            # If the requested number of users is equal to the total number of users
            print("All users are selected") # Select all users
            return self.users

        num_users = min(num_users, len(self.users))
        # Randomly select {num_users} users without replacement from the list of users
    ...
    def evaluate(self, iter, save=True, selected=False):
        ...
        return self.best_accu,self.best_loss
